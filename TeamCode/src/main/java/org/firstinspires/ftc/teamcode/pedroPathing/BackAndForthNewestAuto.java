...
@Autonomous(name = "Back and Forth Newest Auto", group = "Official")
public class BackAndForthNewestAuto extends OpMode {
    ...
    */
    ...
    private final Pose collectPickup1Pose = new Pose(133, 7, Math.toRadians(0)); // collect 1st set of artifacts in loading zone
    ...
    private final Pose collectPickup3ControlPose = new Pose(92, 46.5); //
    private final Pose collectPickup3Pose = new Pose(131.5, 37, Math.toRadians(0));
    private final Pose collectExtraAPose = new Pose(133, 10, Math.toRadians(15)); //
    private final Pose collectExtraReloadPose = new Pose(118, 18, Math.toRadians(20)); //
    private final Pose collectExtraBPose = new Pose(134, 20, Math.toRadians(20)); //
    private Pose parkPose = new Pose(120, 12, Math.toRadians(0)); //

    PathChain shootPreload, collectPickup1, shootPickup1, collectPickup3, shootPickup3, collectExtra, shootExtra, park;
    public void buildPaths() {
        if (colorAlliance == 1) { // red (MIRROR ALL CHANGES IN BLUE)
            ...
            collectPickup3 = follower.pathBuilder()
                    .addPath(new BezierCurve(shootPose, collectPickup3ControlPose, collectPickup3Pose))
                    .setLinearHeadingInterpolation(shootPose.getHeading(), collectPickup3Pose.getHeading())
                    .build();
            shootPickup3 = follower.pathBuilder()
                    .addPath(new BezierLine(collectPickup3Pose, shootPose))
                    .setLinearHeadingInterpolation(collectPickup3Pose.getHeading(), shootPose.getHeading())
                    .build();
            ...
            collectPickup3 = follower.pathBuilder()
                    .addPath(new BezierCurve(shootPose, collectPickup3ControlPose.mirror(), collectPickup3Pose.mirror()))
                    .setLinearHeadingInterpolation(shootPose.getHeading(), collectPickup3Pose.mirror().getHeading())
                    .build();
            shootPickup3 = follower.pathBuilder()
                    .addPath(new BezierLine(collectPickup3Pose.mirror(), shootPose))
                    .setLinearHeadingInterpolation(collectPickup3Pose.mirror().getHeading(), shootPose.getHeading())
                    .build();
    ...
    public void autonomousPathUpdate() {
        switch(pathState) {
            ...
            case 4:
                if (!follower.isBusy()) {
                    follower.followPath(shootPickup3);
                    setPathState(11);
                }
            case 11:
                if (!follower.isBusy()) {
    ...
    public void loop() {
        ...
        if (prioritizePark) {
            if (opmodeTimer.getElapsedTimeSeconds() > 26 && opmodeTimer.getElapsedTimeSeconds() < 26.5) {
                setPathState(1000);
            }
        } else {
            if (opmodeTimer.getElapsedTimeSeconds() > 24 && opmodeTimer.getElapsedTimeSeconds() < 24.5) {
                setPathState(1000);
            }
        }
